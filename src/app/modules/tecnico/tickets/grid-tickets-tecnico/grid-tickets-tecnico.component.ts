...
@Component({
  selector: 'app-grid-tickets-tecnico',
  templateUrl: './grid-tickets-tecnico.component.html',
  styleUrls: ['./grid-tickets-tecnico.component.css']
})
export class GridTicketsTecnicoComponent extends BaseComponent implements OnInit {
  usuario: any;
  ...
  ticketsVencidos: any[] = [];
  ticketActualizando: string | null = null;
  ...
  secuenciaEstatus: string[] = ['Nuevo', 'Asignado', 'En proceso', 'Resuelto', 'Cerrado', 'Rechazado'];
  ...
  mostrandoModalRechazo: boolean = false;
  ticketSeleccionadoParaRechazo: any = null;
  motivoRechazo: string = '';

  private readonly STORAGE_KEYS = {
    VISTA: 'tickets_tecnico_preferencia_vista',
    ITEMS_POR_PAGINA: 'tickets_tecnico_items_por_pagina',
    ORDENAMIENTO_COLUMNAS: 'tickets_tecnico_ordenamiento_columnas',
    COLUMNA_ACTIVA: 'tickets_tecnico_columna_activa'
  };
  ...
  async cargarTickets() {
    try {
      ...
      const ticketsAsignados = await this.ticketsService.getTicketsAsignados(this.usuario.id);
      const ids = new Set();
      this.tickets = ticketsAsignados.filter(ticket => {
  ...
  procesarTickets() {
    ...
    this.tickets.forEach(ticket => {
      if (ticket.estatus) {
        ...
        if (estatusMap[estatusLower]) {
          ticket.estatus = estatusMap[estatusLower];
        }
      }
      
      ticket.fechaCreacionFormatted = this.formatFecha(ticket.fechaCreacion);
      ticket.fechaLimiteFormatted = ticket.fechaLimite ? this.formatFecha(ticket.fechaLimite) : 'Sin fecha límite';
  ...
  verDetalle(ticket: any, event?: Event) {
    ...
    if (ticket.firestoreId) {
      this.router.navigate(['/tecnico/tickets/tickets', ticket.firestoreId]);
    }
  }
  ...
  async aceptarTicket(ticket: any, event: Event) {
    event.stopPropagation();
    
    if (!this.usuario || !this.usuario.id) {
      this.handleAlertType('ERROR', 'No tienes permiso para aceptar este ticket');
      return;
    }
    
    this.confirmationService.confirm({
      message: `¿Aceptar el ticket "${ticket.titulo}"?<br>
              <small>Al aceptar, tomas responsabilidad de resolverlo.</small>`,
      ...
      accept: async () => {
        await this.confirmarAceptacion(ticket);
      }
    });
  }

  async confirmarAceptacion(ticket: any) {
    this.ticketActualizando = ticket.id;
    
    try {
      const hoy = new Date();
      
      // Crear el registro de cambio de estatus
      ...
      // Obtener el historial existente o inicializarlo
      const historialExistente = ticket.historialCambiosEstatus || [];
      const historialActualizado = [...historialExistente, registroCambio];
      
      const resultado = await this.ticketsService.aceptarTicket(
        ticket.firestoreId,
        this.usuario.id,
      ...
      if (resultado) {
        const indice = this.tickets.findIndex(t => t.id === ticket.id);
        if (indice !== -1) {
          this.tickets[indice].estatus = 'Asignado';
          this.tickets[indice].fechasEstatus = {
            ...this.tickets[indice].fechasEstatus,
            fechaAsignado: hoy
          };
          this.tickets[indice].fechaModificacion = hoy;
          this.tickets[indice].ultimoCambioEstatus = registroCambio;
          this.tickets[indice].historialCambiosEstatus = historialActualizado;
          
          this.procesarTickets();
          this.aplicarOrdenamientoYFiltros();
          
          this.handleAlertType('SUCCESS', `Ticket aceptado por ${registroCambio.realizadoPor.nombre}`);
        }
      }
    } catch (error: any) {
      this.handleAlertType('ERROR', error.message || 'Error al aceptar el ticket');
    } finally {
      this.ticketActualizando = null;
    }
  }

  rechazarTicket(ticket: any, event: Event) {
    event.stopPropagation();
    
    if (!this.usuario || !this.usuario.id) {
      this.handleAlertType('ERROR', 'No tienes permiso para rechazar este ticket');
      return;
    }
    
    this.ticketSeleccionadoParaRechazo = ticket;
    this.motivoRechazo = '';
    this.mostrandoModalRechazo = true;
  }

  async confirmarRechazo() {
    if (!this.ticketSeleccionadoParaRechazo || !this.motivoRechazo.trim()) {
      this.handleAlertType('WARNING', 'Por favor, especifica un motivo para el rechazo');
      return;
    }
    
    this.ticketActualizando = this.ticketSeleccionadoParaRechazo.id;
    ...
    try {
      ...
      const registroCambio = {
        fecha: hoy,
        estatusAnterior: this.ticketSeleccionadoParaRechazo.estatus,
      ...
      // Obtener el historial existente o inicializarlo
      const historialExistente = this.ticketSeleccionadoParaRechazo.historialCambiosEstatus || [];
      const historialActualizado = [...historialExistente, registroCambio];
      
      const resultado = await this.ticketsService.rechazarTicket(
        this.ticketSeleccionadoParaRechazo.firestoreId,
        this.usuario.id,
        this.usuario.nombreCompleto || `${this.usuario.nombre} ${this.usuario.apellidoPaterno}`,
        this.motivoRechazo
      );
      ...
      if (resultado) {
        // Actualizar el ticket en la lista local antes de eliminarlo (para preservar historial si es necesario)
        const indice = this.tickets.findIndex(t => t.id === this.ticketSeleccionadoParaRechazo.id);
        if (indice !== -1) {
          this.tickets[indice].ultimoCambioEstatus = registroCambio;
          this.tickets[indice].historialCambiosEstatus = historialActualizado;
        }
        ...
        this.tickets = this.tickets.filter(t => 
          t.id !== this.ticketSeleccionadoParaRechazo.id
        );
        
        this.procesarTickets();
        this.aplicarOrdenamientoYFiltros();
        
        this.handleAlertType('SUCCESS', `Ticket rechazado por ${registroCambio.realizadoPor.nombre} y reasignado a otro técnico`);
      }
    } catch (error: any) {
      this.handleAlertType('ERROR', error.message || 'Error al rechazar el ticket');
    } finally {
      this.ticketActualizando = null;
      this.mostrandoModalRechazo = false;
      this.ticketSeleccionadoParaRechazo = null;
      this.motivoRechazo = '';
    }
  }
  ...
  cambiarEstatus(ticket: any, event: Event) {
    event.stopPropagation();
    
    if (!this.usuario || !this.usuario.id) {
      this.handleAlertType('ERROR', 'No tienes permiso para cambiar el estatus');
      return;
    }

    const estatusActual = ticket.estatus;
    const indiceActual = this.secuenciaEstatus.indexOf(estatusActual);
    
    if (indiceActual === -1) {
      this.handleAlertType('WARNING', 'No se puede cambiar el estatus de este ticket');
      return;
    }

    const siguienteIndice = indiceActual + 1;
    
    if (siguienteIndice >= this.secuenciaEstatus.length) {
      this.handleAlertType('SUCCESS', 'El ticket ya está en el estatus final');
      return;
    }

    const nuevoEstatus = this.secuenciaEstatus[siguienteIndice];
    
    this.confirmationService.confirm({
      message: `¿Estás seguro de cambiar el estatus de "${ticket.titulo}" a "${nuevoEstatus}"?`,
      ...
      rejectButtonStyleClass: 'p-button-secondary',
      accept: () => {
        this.confirmarCambioEstatus(ticket, nuevoEstatus);
      }
    });
  }

  async confirmarCambioEstatus(ticket: any, nuevoEstatus: string) {
    this.ticketActualizando = ticket.id;
    ...
    try {
      ...
      const fechasEstatusActualizadas = { 
        ...(ticket.fechasEstatus || {}),
        [fechaKey]: hoy 
      };
      
      // Crear el registro del cambio de estatus
      const registroCambio = {
        fecha: hoy,
        estatusAnterior: ticket.estatus,
      ...
      await this.ticketsService.actualizarTicket(ticket.firestoreId, datosActualizacion);
      
      const indice = this.tickets.findIndex(t => t.id === ticket.id);
      if (indice !== -1) {
        this.tickets[indice].estatus = nuevoEstatus;
        this.tickets[indice].fechasEstatus = fechasEstatusActualizadas;
        this.tickets[indice].fechaModificacion = hoy;
        this.tickets[indice].ultimoCambioEstatus = registroCambio;
        this.tickets[indice].historialCambiosEstatus = historialActualizado;
      ...
      const mensajeError = error.message || 'Error desconocido al cambiar el estatus';
  ...
  cancelarTicket(ticket: any, event: Event) {
    event.stopPropagation();
    
    this.confirmationService.confirm({
      message: `¿Estás seguro de cancelar el ticket "${ticket.titulo}"?<br>
      ...
      rejectButtonStyleClass: 'p-button-secondary',
      accept: () => {
        this.confirmarCancelacion(ticket);
      }
    });
  }

  async confirmarCancelacion(ticket: any) {
    this.ticketActualizando = ticket.id;
    ...
    try {
      ...
      const registroCancelacion = {
        ...
        motivo: 'Cancelado por el técnico'
      };
      ...
      const datosActualizacion = {
        ...
        fechasEstatus: fechasEstatusActualizadas,
        fechaModificacion: hoy,
        motivoCancelacion: 'Cancelado por el técnico',
      ...
      const indice = this.tickets.findIndex(t => t.id === ticket.id);
      if (indice !== -1) {
        this.tickets[indice].estatus = 'Cancelado';
        this.tickets[indice].fechasEstatus = fechasEstatusActualizadas;
        this.tickets[indice].fechaModificacion = hoy;
        this.tickets[indice].motivoCancelacion = 'Cancelado por el técnico';
        this.tickets[indice].canceladoPor = registroCancelacion.realizadoPor;
        this.tickets[indice].fechaCancelacion = hoy;
        this.tickets[indice].ultimoCambioEstatus = registroCancelacion;
        this.tickets[indice].historialCambiosEstatus = historialActualizado;
  ...
  getSiguienteEstatus(ticket: any): string {
    const estatusActual = ticket.estatus;
    const indiceActual = this.secuenciaEstatus.indexOf(estatusActual);
    
    if (indiceActual === -1 || indiceActual >= this.secuenciaEstatus.length - 1) {
      return '';
    }
    
    return this.secuenciaEstatus[indiceActual + 1];
  }

  getTextoBotonEstatus(ticket: any): string {
    const siguienteEstatus = this.getSiguienteEstatus(ticket);
    ...
    return textos[ticket.estatus] || `Cambiar a ${siguienteEstatus}`;
  }

  getClaseBotonEstatus(ticket: any): string {
    const estatusActual = ticket.estatus;
    
    switch (estatusActual) {
      case 'Nuevo':
        return 'btn-estatus-nuevo';
      case 'Asignado':
        return 'btn-estatus-asignado';
      case 'En proceso':
        return 'btn-estatus-proceso';
      case 'Resuelto':
        return 'btn-estatus-resuelto';
      default:
        return 'btn-estatus-default';
    }
  }

  getIconoBotonEstatus(ticket: any): string {
    const siguienteEstatus = this.getSiguienteEstatus(ticket);
    ...
    const iconos: {[key: string]: string} = {
      'Nuevo': 'pi pi-user-check',
      'Asignado': 'pi pi-play',
      'En proceso': 'pi pi-check-circle',
      'Resuelto': 'pi pi-lock'
    };
    
    return iconos[ticket.estatus] || 'pi pi-arrow-right';
  }

  puedeCambiarEstatus(ticket: any): boolean {
    if (!this.usuario || !this.usuario.id) {
      return false;
    }

    if (ticket.asignadoA?.id !== this.usuario.id) {
      return false;
    }

    if (ticket.estatus === 'Cancelado' || ticket.estatus === 'Cerrado' || ticket.estatus === 'Rechazado') {
      return false;
    }
    
    return this.secuenciaEstatus.includes(ticket.estatus);
  }
}