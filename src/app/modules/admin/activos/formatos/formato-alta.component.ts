...
@Component({
  selector: 'app-formato-alta',
  template: `
    <div class="formato-container">
      <div class="loading" *ngIf="cargando">
        <i class="pi pi-spinner pi-spin"></i>
        <p>Generando formato de alta...</p>
      </div>
    </div>
  `,
  styles: [`
...
export class FormatoAltaComponent extends BaseComponent implements OnInit {
  ...
  async generarFormato() {
    try {
      ...
      const doc = this.reportesService.generarFormatoAlta(activo, nombreUsuario);
      
      doc.save(`formato-alta-${activo.nombre}-${new Date().getTime()}.pdf`);
